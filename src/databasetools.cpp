...
bool DatabaseTools::create_new_db()
{
    ...
    operations.append("CREATE INDEX index_vocabulary_percentage_correct ON vocabulary(CAST(number_correct AS REAL)/CAST(number_asked AS REAL))");
...
bool DatabaseTools::test_and_update_db()
{
    ...
    case 4:
        ...
        s = "CREATE INDEX index_vocabulary_percentage_correct ON vocabulary(CAST(number_correct AS REAL)/CAST(number_asked AS REAL))";
        if(!query.exec(s))
        {
            QString error = s;
            error.append(": ").append(query.lastError().text());
